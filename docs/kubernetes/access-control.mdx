...
To let a sandbox create and manage its own child sandboxes, opt in at creation
time with `OPENSHELL_DELEGATION_TOKEN_FILE=/run/openshell-delegation/token`.
The supervisor mints a separate, parent-bound delegation token at that fixed
path and refreshes it while the sandbox runs. The workload never receives the
supervisor JWT. The delegation token can manage only children of that parent;
it cannot list the workspace or act as a general sandbox credential.
