...
A policy has static sections `filesystem_policy`, `landlock`, `process`, and `ssh` that are locked at sandbox creation, and dynamic `network_policies` and `network_middlewares` sections that are hot-reloadable on a running sandbox.
...
# Static: disabled unless a root is explicitly configured. Permits only
# <root>/<private child>/<socket name> reverse Unix-socket forwarding over SSH.
ssh:
...
| `ssh` | Static | Disabled by default. When `remote_streamlocal_forward_root` is set, the sandbox SSH server accepts only a reverse Unix-socket forward at `<root>/<private child>/<socket name>`; it rejects TCP and paths outside that root. The child directory must already exist, be non-symlinked, and have no group or other permissions. See the [Policy Schema Reference](/reference/policy-schema#ssh). |
