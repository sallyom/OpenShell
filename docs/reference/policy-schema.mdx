...
| `ssh` | object | No | Static | Opts in to a narrowly scoped reverse Unix-socket forwarding path. |
...
Controls the optional reverse Unix-socket forwarding capability of the sandbox SSH server. It is disabled unless this section sets a root.

| Field | Type | Required | Description |
|---|---|---|---|
| `remote_streamlocal_forward_root` | string | No | Absolute root beneath which an SSH client may request a Unix-socket listener. |

The requested socket must be `<root>/<single-private-directory>/<socket-name>`. The child directory must already exist, must not be a symlink, and must grant no group or other permissions. Directory and socket names are restricted to ASCII letters, digits, `.`, `_`, and `-`. OpenShell rejects TCP forwarding and a request outside this root. The sandbox removes the socket when the forwarding request is cancelled or its SSH connection ends.
